...
class LibrarianDB:
    ...
    async def init(self):
        async with aiosqlite.connect(self.path) as db:
            ...
            # 유저별 감정 (user_filter, user_lovely, user_trust)
            await db.execute("""
    ...
    async def update_emotion(self, changes: dict, target_user_id: str = None,
        ...
        async with aiosqlite.connect(self.path) as db:
            ...
            if user_changes and target_user_id:
                ...
                row = await cursor.fetchone()
                current = dict(row) if row else {"filter": self.NEUTRAL, "lovely": self.NEUTRAL, "trust": self.NEUTRAL,
                                                  "interaction_count": 0}
                ...
                await db.execute("""
                    INSERT INTO user_emotion (user_id, user_name, filter, lovely, trust, interaction_count, last_interaction)
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                    ON CONFLICT(user_id) DO UPDATE SET
                        user_name=excluded.user_name,
                        filter=excluded.filter, lovely=excluded.lovely, trust=excluded.trust,
                        interaction_count=excluded.interaction_count,
                        last_interaction=excluded.last_interaction
                """, (target_user_id, target_user_name,
                      current["filter"], current["lovely"], current["trust"],
