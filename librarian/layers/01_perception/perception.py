...
perception_declarations = [
    ...
    types.FunctionDeclaration(
        ...
        parameters=types.Schema(
            type="OBJECT",
            properties={
                "urls": types.Schema(type="ARRAY", items=types.Schema(type="STRING"), description="확인할 URL 배열. 예: ['https://...']"),
            },
            required=["urls"],
        ),
    ),
        ...
        for part in response.candidates[0].content.parts:
            ...
            # recognize_media (배열 지원)
                ...
                for att_idx in indices:
                    ...
                    tool_results.append(f"미디어 인식 ({att_idx}): {media_result or '인식 실패'}")

            # recognize_link (배열 지원)
