...
## v3 (429f117, 2026-03-30)

프로젝트 구조 전면 개편 + AI 판단 위임. 강제 트리거/필터를 전부 제거하고 모델에게 맡김.
...
**AI 단순화 (v2 로그 분석 기반):**
- v2에서 `_clean_reply`가 커스텀 이모지(`<:name:id>`), 채널 멘션(`<#id>`), 유저 멘션(`<@id>`)을 삭제해서 정상 답변이 빈 응답으로 처리되는 문제가 있었음 → `_clean_reply` 전체 제거
- 키워드 기반 웹 검색 트리거("검색해", "날씨 알려" 등)가 파일 요청과 충돌하고, 독립 호출이라 맥락 없이 "잠깐만 기다려봐"만 반복 → 키워드 트리거 제거, AI가 web_search 도구를 자발적으로 호출하면 function calling 루프 안에서 Google Search grounding 처리
- 기억 자동 저장(키워드 트리거 + 설명식 패턴)이 쓰레기 학습과 부정적 콘텐츠 노출의 원인 → 자동 저장 전부 제거, AI가 save_memory 도구를 자발적으로 호출할 때만 저장
- 채널 대화 30건을 시스템 프롬프트에 넣는 것이 잡담 모드로 빠지게 하고 사서 역할을 방해 → 채널 맥락 제거, Gemini 자체 히스토리(20턴)에 의존
- 3단계 리트라이(기본→맥락 제거→웹 폴백)의 코드 중복과 2차 재시도 파싱 버그 → 리트라이 전부 제거, 1회 호출로 단순화
- 모델을 gemini-3.1-flash-lite-preview로 업그레이드 (Intelligence 16→34)해서 AI 자체 판단력 향상

### 주요 변경

- 모델: gemini-2.5-flash-lite → gemini-3.1-flash-lite-preview
- 네이밍: `ai/` → `librarian/`, `uploads/` → `files/`, `ai_bot.log` → `logs/bot.log`
- **제거**: `_clean_reply`, 웹 검색 키워드 트리거, 웹 검색 폴백, 기억 자동 저장, 채널 대화 맥락, 채널/글로벌 버퍼, 3단계 리트라이, 반복 비교
- **추가**: `logs/server.log` (서버 전체 메시지 로그), `logs/chat.jsonl` (구조화된 대화 로그)
- 히스토리: 6턴 → 20턴
- `_call_gemini` 메서드로 추출 (config 파라미터화)
