...
customs = [
    ...
    # 밈
    ("meme", "HFSP(Have Fun Staying Poor): 비트코인 안 사는 사람에게 하는 말."),
    ("meme", "Few understand: 비트코인의 진가를 이해하는 사람이 적다는 뜻."),
    ("meme", "Number go up: 비트코인 가격 상승을 축하하는 밈."),
    ("meme", "Laser eyes: 비트코인 맥시들이 프사에 레이저 눈을 합성하는 문화."),
    ("meme", "We're all gonna make it(WAGMI): 다 같이 잘 될 거라는 뜻. 비트코인 커뮤니티 격려 문구."),
    ("meme", "Not gonna make it(NGMI): WAGMI의 반대. 비트코인 안 사는 사람에게."),
    ("meme", "Rug pull: 프로젝트 운영자가 자금을 가지고 도망하는 사기. 주로 알트코인에서 발생."),
    ("meme", "Honey badger don't care: 비트코인은 뭐가 됐든 신경 안 쓰고 작동한다는 밈."),
    ("meme", "Bitcoin fixes this: 세상의 모든 문제에 '비트코인이 이걸 고친다'고 답하는 밈."),
    ("meme", "이국주: 시타델 커뮤니티에서 봇을 놀릴 때 쓰는 밈. 무시하거나 가볍게 받아치면 됨."),

    # 잡학
    ("trivia", "비트코인 피자 2판의 현재 가치는 수십억 달러. 역사상 가장 비싼 피자."),
    ("trivia", "사토시 나카모토의 정체는 아직까지 밝혀지지 않았다. 할 피니, 닉 사보 등이 후보."),
    ("trivia", "비트코인 블록체인의 첫 거래 메시지에 영국 은행 구제금융 뉴스 헤드라인이 담겨 있다."),
    ("trivia", "비트코인 채굴에 사용되는 에너지는 아이슬란드 전체 소비량보다 많다."),
    ("trivia", "비트코인 주소의 경우의 수(2^160)는 지구의 모래알보다 많다."),
    ("trivia", "사토시 나카모토가 보유한 것으로 추정되는 100만 BTC는 한 번도 움직인 적 없다."),
]
