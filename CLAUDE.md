...
```
L1 Perception (temp 0.3)
  맥락 관찰. 감정 수치를 자연어로 해석. 이전 소견 최우선 전달.
  지시하지 않음. 판단 재료만 넘김.
  도구 없음.
      |
L2 Functioning (temp 0.5)
  도구 실행. L1 결과 참조.
  도구: search, deliver, attach, web_search, recognize_media, recognize_link, memorize_alias, forget_alias
      |
L3 Character (temp 1.2)
  대사 생성. 역할만 정의, 성격은 L1이 넘긴 감정 해석에 의해 동적 결정.
  도구 없음. 히스토리 포함.
      |
L4 Postprocess (temp 0.1)
  시스템 용어 제거, 자연어 정제. 과잉 연출 제거.
  도구 없음.
      |
L5 Evaluation (temp 0.3, 백그라운드)
  적합성 판단 + 감정 변화 + 소견.
  도구: feel, memorize, forget
  전체 맥락(L1+L2 결과) 받음. 명령이 아닌 우려 표현.
```
...
L4 Postprocess:
- 01_role.txt -- 검수자 역할, 시스템 용어 제거 목록, 2문장 제한

L5 Evaluation:
- 01_role.txt -- 평론가 역할, 적합성 판단, 소견은 우려 표현
- 02_emotion.txt -- 6축 감정 판정 기준 + 변화량 가이드
- 03_feedback.txt -- 소견 작성 규칙
...
- Character(L3)만 히스토리 사용. 다른 레이어는 매번 독립 호출.
...
- self_capacity -- 여력. 소모 자원, 명시적 회복만 인정
