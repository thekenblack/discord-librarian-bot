...
- 역할이 다르고 토큰도 다르다
- main.py가 하나의 asyncio 이벤트 루프에서 두 봇을 동시 실행
- 같은 프로세스이므로 상호 직접 호출 가능 (ai_bot.library_bot_client, library_bot.ai_bot_client)
- startup.py가 main.py 프로세스를 관리하고, exit code 42로 git pull + 전체 재시작
...
```
L1 Perception (채널별 히스토리 5건)
  맥락 관찰 + 의도 파악 + 로컬 검색(싼 호출만).
  감정 수치를 자연어로 해석. 이전 소견 최우선 전달.
  공통 컨텍스트(원본) 직접 참조.
  도구: search
      |
L2 Execution (히스토리 없음)
  L1 관찰을 로컬 데이터로 보충 + 비싼 실행 전담.
  공통 컨텍스트(원본) + L1 분석 참조.
  도구: recognize_media, recognize_link, deliver, attach, web_search, gift_user
      |
L3 Character (유저별 히스토리 5건)
  순수 연기자. 공통 컨텍스트 원본을 직접 보지 않음.
  L1 분석(해석본) + L2 보고만 받아서 대사 생성. @닉네임 형태로 출력.
  히스토리에 raw_reply(L4 변환 전) 저장.
  도구 없음.
      |
L4 Postprocess (히스토리 없음)
  멘션 퍼지 매칭(@닉네임 -> <@ID>) + 행동 정합성 검증(L2 보고 기준).
  L3 대사 + L2 보고 + mention_map 참조.
  L4 직후 코드 검증: mention_map에 없는 <@ID>는 Discord API로 실존 확인, 무효 시 @이름으로 복원.
  도구 없음.
      |
L5 Evaluation (단일 히스토리 5건, 큐 워커, 디바운스 30분/5건)
  감정 변화 + 소견 + 유저/채널 요약 갱신.
  도구: feel, memorize, forget, memorize_alias, forget_alias, update_summary, update_channel_summary
```

전 레이어 temp 1.0, thinking MINIMAL 고정.
유저 체감 대기: L1 -> L2 -> L3 -> L4 (직렬 4콜, 각 1회). L5는 응답 전송 후 큐 워커가 순차 처리.
L1~L4: Gemini 3.1-flash-lite. L5: Gemini 3.0-flash. config.json에서 레이어별 모델 개별 지정 가능 (model_l2, model_l4).

공통 컨텍스트:
- _ask_gemini 시작 시 DB 1회 조회 (bot_emotion, user_emotion, summary, balance, catalog, memories 등)
- L1/L2: 공통 컨텍스트 원본 직접 참조
- L3: 원본 참조 안 함. L1/L2의 해석만 받음 (배우는 설정집이 아니라 대본을 읽는다)

@닉네임 통일:
- 맥락, L1/L2 출력, L3 대사 전부 @닉네임 형태
- _mention_map에 닉네임->ID 누적
- L4가 mention_map 기반으로 <@ID> 변환
...
5. DB 백업 (최근 5개 유지)
6. 봇 프로세스 관리 (main.py 단일 프로세스. 재시작 신호 42 -> git pull -> pip install -> 전체 재시작)
...
L1 Perception:
- 01_role.txt -- 관찰자 역할, 의도 파악, 지식 판단, 이전 소견 최우선, 도구 현황 보고
- 02_emotion.txt -- 감정 수치 해석 가이드 (6축, 각 축 독립성 명시)

L2 Execution:
- 01_role.txt -- 실행기 역할 + 로컬 데이터 보충 판단
- 02_library.txt -- 도서관 안내 (카탈로그/기억은 공통 컨텍스트에 포함)
- 03_tools.txt -- 도구 설명 (recognize_media/link, deliver, attach, web_search, gift_user)
- 05_instruction.txt -- 판단 보충 + 도구 현황 보고 + 정보 전달

L3 Character:
- 01_persona.txt -- 역할만 (시타델 도서관 사서). 성격 없음
- 02_appearance.txt -- 외양
- 03_behavior.txt -- 출력 규칙 + 컨텍스트 활용법 + @닉네임 + 유니코드 수식 + 디스코드 마크다운

L4 Postprocess:
- 01_role.txt -- 멘션 퍼지 매칭 + 행동 정합성 검증 (L2 보고 대조)
...
레이어별 히스토리 스코프:
- L1 Perception: 채널별 히스토리 (channel_id -> history). MAX=5
- L2 Execution: 히스토리 없음. 단발 1회 호출
- L3 Character: 유저별 히스토리 (user_id -> history). MAX=5. trim 시 function_call/response 쌍 보장. raw_reply(L4 변환 전) 저장
- L4 Postprocess: 히스토리 없음. L3 대사 + L2 보고(instruction) + mention_map 참조
- L5 Evaluation: 단일 히스토리 (전 채널/유저 공유). MAX=5. 큐 워커가 순차 처리
...
Perception(L1) 도구 -- 싼 호출 (로컬):
| 도구 | 용도 |
|---|---|
| search(keyword) | 지식+기억+도서+웹+미디어+유저감정 통합 로컬 검색. 뉴스/날씨도 가능 |

Execution(L2) 도구 -- 비싼 호출 (API/실행):
| 도구 | 용도 |
|---|---|
| recognize_media(indices) | 이미지/PDF 인식. 배열로 여러 개 동시 (file_hash 중복 방지) |
| recognize_link(urls) | URL 인식. 배열로 여러 개 동시 (이미지 URL은 동기, 나머지 백그라운드) |
| deliver(file_id) | 파일 전송 |
| attach(media_id 또는 url_id) | 저장된 미디어/URL 첨부 전송. ID 모르면 호출 금지 |
| web_search(query) | 웹 검색 (L1 검색 결과 불충분 시만 사용) |
| gift_user(item_id) | 유저에게 선물. 잔고 확인 후 실행. 실패 시 [선물 실패] 보고 |

Evaluation(L5) 도구:
| 도구 | 용도 |
|---|---|
| feel(...) | 감정 변화 기록. targets 배열로 여러 유저 동시. message_id 중복 방지 |
...
- 단일 API 키
- `_call_gemini`: async (run_in_executor 내장), 재시도 3회, 1초 간격, model 파라미터로 레이어별 모델 지정 가능
- 이벤트 루프 블로킹 없음

### 로그

- `bot.YYYY-MM-DD.log` -- 레이어별 타이밍 ([L1 Perception], [L2 Execution], [Character], [Postprocess], [Evaluation])
...
| 커맨드 | 설명 |
|---|---|
| /charge [amount] | Lightning 인보이스로 잔고 충전 |
| /buy | 사서봇에게 선물 (잔고 부족 시 부족분 인보이스 자동 발행) |
| /balance | 내 잔고 확인 |
...
| /admin charge [user] | 잔고 직접 충전 (모달, user 생략 시 본인) |
