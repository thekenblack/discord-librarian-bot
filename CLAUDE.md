...
L1 Perception (temp 0.3, 채널별 히스토리 5건)
  맥락 관찰 + 로컬 정보 수집. 감정 수치를 자연어로 해석. 이전 소견 최우선 전달.
  유저/채널 요약(DB)을 읽어서 장기 맥락 반영.
  도구: search, recognize_media, recognize_link
      |
L2 Functioning (temp 0.5, 히스토리 없음)
  실행 전담. L1 결과 참조. 검색 결과 불충분 시 웹 검색. 1회 호출.
  도구: deliver, attach, web_search, gift_user
      |
...
L4 Postprocess (temp 0.1, 히스토리 없음)
  최소한 손질. 시스템 흔적 제거, 멘션/채널/역할/이모지 교정. 원문 보존 우선.
  도구 없음.
      |
...
전 레이어 Gemini 메인 호출 1회. L1/L2의 도구 실행(Google Search, 미디어 인식 등)은 별도 경량 호출.
...
L4 Postprocess:
- 01_role.txt -- 최소 손질. 시스템 흔적 제거, 멘션/채널 교정. 원문 보존 우선
...
- L2 Functioning: 히스토리 없음. 단발 1회 호출
- L3 Character: 유저별 히스토리 (user_id -> history). MAX=5. trim 시 function_call/response 쌍 보장
...
| gift_user(item_id) | 유저에게 선물 (감정적으로 주고 싶을 때) |
